...
void run_statistical_validation(const float* d_P_market, const float* d_f_market) {
    printf("\n");
    printf("statistical validation via confidence intervals...");
    printf("\n");

    float S1 = 5.0f, S2 = 10.0f, K = expf(-0.1f);
    const int N_RUNS = 20;

    float vega_samples[N_RUNS];

    printf("Running %d independent Monte Carlo simulations...\n", N_RUNS);

    unsigned long base_seed = (unsigned long)time(NULL) * 2;

    for (int run = 0; run < N_RUNS; run++) {
          curandState *d_states_run;
        cudaMalloc(&d_states_run, N_PATHS * sizeof(curandState));

        unsigned long seed = base_seed + run * 982451653UL;

        init_rng<<<NB, NTPB>>>(d_states_run, seed);
        cudaDeviceSynchronize();
        ...
        simulate_sensitivity<<<NB, NTPB>>>(d_sens_sum, d_states_run, S1, S2, K,
                                            d_P_market, d_f_market);
        cudaDeviceSynchronize();

        float sum;
        cudaMemcpy(&sum, d_sens_sum, sizeof(float), cudaMemcpyDeviceToHost);
        vega_samples[run] = sum / N_PATHS;

        cudaFree(d_sens_sum);
        cudaFree(d_states_run);
        if ((run + 1) % 5 == 0) {
    ...
    float variance = 0.0f;
    for (int i = 0; i < N_RUNS; i++) {
        float diff = vega_samples[i] - mean;
        variance += diff * diff;
    }
    variance /= (N_RUNS - 1);
    ...
    printf("  Min:  %.6f\n", *std::min_element(vega_samples, vega_samples + N_RUNS));
    printf("  Q1:   %.6f\n", vega_samples[N_RUNS/4]);
    printf("  Med:  %.6f\n", vega_samples[N_RUNS/2]);
    printf("  Q3:   %.6f\n", vega_samples[3*N_RUNS/4]);
    printf("  Max:  %.6f\n", *std::max_element(vega_samples, vega_samples + N_RUNS));

    printf("\n");
    printf("Interpretation:\n");
    printf("  • We are 95%% confident the true vega lies in [%.6f, %.6f]\n", ci_lower, ci_upper);
    printf("  • Any difference < %.6f is within sampling noise\n", 2 * std_error);
    printf("  • CV = %.4f%% indicates %s precision\n", cv_percent,
           cv_percent < 0.5f ? "excellent" : cv_percent < 1.0f ? "good" : "acceptable");

    FILE* csv = fopen("data/vega_bootstrap.csv", "w");
    if (csv) {
        fprintf(csv, "run,vega\n");
        for (int i = 0; i < N_RUNS; i++) {
            fprintf(csv, "%d,%.8f\n", i+1, vega_samples[i]);
        }
        fclose(csv);
        printf("\nSaved data/vega_bootstrap.csv\n");
    }

    FILE* stats = fopen("data/vega_statistics.txt", "w");
    if (stats) {
        fprintf(stats, "VEGA ESTIMATE STATISTICAL VALIDATION\n");
        fprintf(stats, "=====================================\n\n");
        ...
        fprintf(stats, "  Mean Vega:         %.6f\n", mean);
        fprintf(stats, "\n");
        fprintf(stats, "Uncertainty Quantification:\n");
        fprintf(stats, "  Standard Error:    %.6f (%.4f%%)\n", std_error, 100.0f * std_error / mean);
        fprintf(stats, "  95%% CI:             [%.6f, %.6f]\n", ci_lower, ci_upper);
        fprintf(stats, "\n");
        fprintf(stats, "Validation:\n");
        fprintf(stats, "  Differences < %.6f are statistically insignificant\n", 2 * std_error);
        fprintf(stats, "  at the 95%% confidence level (within 2 SE).\n");
        fclose(stats);
        printf("Saved data/vega_statistics.txt\n");
    }
}

void validate_method_agreement(float vega_pw, float vega_fd, float std_error) {
   
    float diff = fabsf(vega_pw - vega_fd);
    float z_score = diff / std_error;
    bool statistically_significant = (z_score > 1.96f);

    printf("Comparing Pathwise vs Finite Difference:\n");
    printf("  Pathwise Vega:      %.6f\n", vega_pw);
    printf("  Finite Diff Vega:   %.6f\n", vega_fd);
    printf("  Absolute Diff:      %.6f\n", diff);
    printf("  Relative Diff:      %.4f%%\n", 100.0f * diff / vega_pw);
    printf("\n");
    printf("Statistical Test (H₀: methods agree):\n");
    printf("  Standard Error:     %.6f\n", std_error);
    printf("  Z-score:            %.4f\n", z_score);
    printf("  Critical value:     1.96 (95%% CI)\n");
    printf("\n");

    if (statistically_significant) {
        printf("  Result: SIGNIFICANT DIFFERENCE (p < 0.05)\n");
        printf("  Methods disagree beyond sampling noise\n");
    } else {
        printf("  Result: NO SIGNIFICANT DIFFERENCE (p > 0.05)\n");
        printf("  Methods agree within Monte Carlo noise\n");
    }

    printf("\n");
    printf("Effect Size Analysis:\n");
    float effect_size_in_se = diff / std_error;
    printf("  Difference = %.2f × SE\n", effect_size_in_se);
    if (effect_size_in_se < 0.5f) {
        printf("  Interpretation: Negligible difference (< 0.5 SE)\n");
    } else if (effect_size_in_se < 1.0f) {
        printf("  Interpretation: Small difference (< 1 SE)\n");
    } else if (effect_size_in_se < 2.0f) {
        printf("  Interpretation: Moderate difference (< 2 SE)\n");
    } else {
        printf("  Interpretation: Large difference (≥ 2 SE)\n");
    }
}
...
printf("\n");
    ...
    char response_stat;
    scanf(" %c", &response_stat);
    if (response_stat == 'y' || response_stat == 'Y') {
    run_statistical_validation(d_P_market, d_f_market);
    float estimated_se = 0.000089f;
    validate_method_agreement(sens_mc, sens_fd, estimated_se);
}
